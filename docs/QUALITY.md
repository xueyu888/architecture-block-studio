...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1905 modules transformed |
| Vitest 快速单元测试 | 191 / 191 通过，25 个 test files；覆盖桌面文件服务、场景路由、viewport auto-pan、片段、组选区、位置化模块创建、最深 Level drop-target 与稳定同深裁决、空子 Level 投影、包括具名 `node/rename` / `node/add` 在内的 Editor 原子合同与真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 68 个文件、27 条唯一依赖全部带 import 证据 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，87 / 87 通过；与 Firefox 在当前最终检出上并行完整回归 173 / 173 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，86 / 86 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，并与 Chromium 共同验证最深内联 Level 命中、空子 Level 第一张卡片、预览与最终 JSON 一致、画布指定位置新建、真实 toolbar drag、五层源码架构、12 个真实模块、27 条 import 依赖及全部 351 个无序线对；其余覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、组选区八向 resize、对齐 / 等距、片段操作、右键菜单、路由编辑、连接预览、五层与 Hub 压力、命令入口和保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮最终完整回归样本中首次可交互 1532 ms、选择全部 400 interfaces / 200 modules 为 773 / 488 ms、模块选择 470 ms、对象菜单 206 ms、直接邻域 / 出站邻域为 655 / 674 ms、多选两个模块 499 ms、两模块成组 resize / Undo 为 1516 / 959 ms、Alt 命中 226 ms、键盘遍历 438 ms、模块搜索与定位 1773 ms、接口筛选 528 ms、接口端点聚焦 529 ms、单模块 Cut + Undo 1583 ms、保存并校验 81 ms；连接预览峰值 3.8 ms、末次 0.3 ms；测得总量 110,664,010 bytes，均为 observation-only 环境观测而非固定预算 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、1000 个 MiniMap 节点与保存保持全量，初始 Canvas DOM 只挂载视口内 200 modules / 430 connections，默认 Hierarchy 为完整 1002 行投影但首批 DOM 仅 40 行；三轮中位数仍是历史趋势证据。本轮 Iteration 100 单次复验为首次可交互 3859 ms、选择全部 2000 interfaces / 1000 modules 为 1164 / 576 ms、十次编辑 3608 ms、对象菜单 86 ms、直接邻域 / 出站邻域为 826 / 677 ms、多选两个模块 601 ms、两模块成组 resize / Undo 为 3421 / 2759 ms、单模块 Cut + Undo 5936 ms、保存并校验 114 ms；连接预览登记全部 1000 个障碍，13 次请求 / 6 次求解 / 7 次缓存命中，峰值 6.4 ms；最终测量内存 179,668,680 bytes。这些是本机 observation，不是固定性能预算 |
...
| 浏览器范围 | Chromium 87 / 87 完整自动化；Firefox 86 / 86 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证，但 Windows 桌面产品不以 WebKit 或移动端为交付目标 |
