...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1899 modules transformed，7.07 秒 |
| Vitest 快速单元测试 | 144 / 144 通过，19 个 test files / 1.47 秒；除既有场景路由、片段与 viewport auto-pan 合同外，覆盖多成员包围框和组 subject 的边缘 / 中心吸附 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，71 / 71 通过；与 Firefox 在当前检出上并行完整回归 141 / 141（4.0 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，70 / 70 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、单卡与多选组边界辅助线、抓取入口一致性、多选原子对齐 / 分布、当前 Level 全选 / 清空、Copy / Paste / Duplicate / Ctrl/⌘ 拖动克隆、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、八条直接手势的边缘自动平移、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、完整障碍预览、gesture session 注册 / 复用 / 清理、pointer 候选 / 取消 / 非法 / no-op / 成功重连、键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮最终完整回归样本中首次可交互 1543 ms、模块选择 506 ms、多选两个模块 916 ms、模块搜索与定位 1814 ms、接口筛选 500 ms、接口端点聚焦 531 ms、保存并校验 84 ms；连接预览一次注册 200 obstacles，15 requests / 7 solves / 8 hits，峰值 4.2 ms、末次 0.5 ms；测得 JS + embedder + backing storage 总量 90,711,018 bytes，均为 observation-only 环境观测而非固定预算 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、1000 个 MiniMap 节点与保存保持全量，初始 Canvas DOM 只挂载视口内 200 modules / 430 connections，默认 Hierarchy 为完整 1002 行投影但首批 DOM 仅 40 行；三轮中位数：首次可交互 2552 ms，十次 Level 编辑 2692 ms、Undo 199 ms、Redo 190 ms；Sources 选择屏外模块并定位到可读尺寸 217 ms，MiniMap 首尾节点跨图定位 63 / 60 ms，四次 Hierarchy 查询、一次增量加载与最终定位 758 ms，接口筛选 131 ms、接口端点与路径定位 182 ms，保存并校验 62 ms；历史交互测得增量 18,264,378 bytes，最终 JS + embedder + backing storage 总量 65,320,494 bytes。Iteration 89 单次压力复验为首次可交互 3856 ms、十次编辑 3447 ms、Undo / Redo 222 / 212 ms、跨图选择 242 ms、多选 685 ms、组参考线拖动 3314 ms、MiniMap 定位 73 ms、搜索 790 ms、保存 69 ms；连接预览一次注册 1000 obstacles，13 requests / 5 solves / 8 hits，峰值 6.7 ms、末次 1.0 ms，并在取消后保留原有 2000 connections |
...
| 编辑闭环 | 新建、模块、端口、模块移动 / 四边四角 resize、当前 Level 全选 / 清空、多选原子对齐 / 分布、Copy / Paste / Duplicate / Ctrl/⌘ 拖动完整子图克隆、选中线路完整聚焦、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、鼠标 / 键盘端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 71 / 71 完整自动化；Firefox 70 / 70 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
