...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1891 modules transformed，7.45 秒 |
| Vitest 快速单元测试 | 109 / 109 通过，17 个 test files，1.61 秒；其中场景路由 11 条覆盖 54-leg 真实设计、120-leg 偏斜压力、五层 Gate 链与不可满足共享端口 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，49 / 49 通过；与 Firefox 在已核验为当前检出的开发服务器上并行完整回归 97 / 97，共 2.9 分钟 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，48 / 48 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、对齐辅助线、多选原子对齐 / 分布与明确边界、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮完整回归样本中首次可交互 1476 ms、模块选择 514 ms、多选两个模块 934 ms、四类模块搜索、一次增量加载并键盘定位 1739 ms、接口筛选 430 ms、接口端点聚焦 579 ms、保存并校验 101 ms；测得 JS + embedder + backing storage 总量 83,311,819 bytes，均为 observation-only 环境观测而非固定预算 |
...
| 浏览器范围 | Chromium 49 / 49 完整自动化；Firefox 48 / 48 产品合同，唯一排除项为 Chromium 专属 CDP heap 采样；WebKit 尚未验证 |
