...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1881 modules transformed，6.91 秒 |
| Vitest 快速单元测试 | 75 / 75 通过，12 个 test files，0.417 秒 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，34 / 34 通过；与 Firefox 在测试自有隔离端口并行完整回归 67 / 67，共 4.3 分钟 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，33 / 33 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、路由、方向 marker、Tooltip、Command Palette 与保存导出 |
...
| 浏览器范围 | Chromium 34 / 34 完整自动化；Firefox 33 / 33 产品合同，唯一排除项为 Chromium 专属 CDP heap 采样；WebKit 尚未验证 |
