...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1915 modules transformed |
| Vitest 快速单元测试 | 214 / 214 通过，31 个 test files；覆盖桌面文件服务、场景路由、viewport auto-pan、片段、组选区、位置化模块创建、最深 Level drop-target、布局 / route / line-jump 引用协调、latest-only Worker 请求队列、完整候选路线证书，以及真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 81 个文件、27 条唯一依赖全部带 import 证据 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与全部路径几何门禁。最终全量样本：首次可交互 1332 ms；全部接口 / 模块选择 476 / 486 ms；组选区 resize 2319 ms，其中 pointer release 84 ms、committed ready 711 ms、Worker 26.1 ms；affected / neighborhood 6 / 27，只重投影 2 nodes / 6 edges；Undo 738 ms、Cut + Undo 1243 ms、保存 57 ms、总测量 112,438,674 bytes。均为 observation-only 环境观测 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、MiniMap 与保存保持全量，Canvas 只裁剪 DOM。当前单次样本：首次可交互 3888 ms、十次编辑 3648 ms；组选区 resize 3222 ms，其中 pointer release 177 ms、committed ready 1343 ms、Worker 246.7 ms；affected / neighborhood 6 / 27，只重投影 2 nodes / 6 edges；Undo 1236 ms、Cut + Undo 1418 ms、保存 123 ms；1000 障碍连接预览峰值 7.1 ms；最终测量 242,798,641 bytes。这些是开发机 observation，不是固定预算 |
