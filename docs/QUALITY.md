...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，74 / 74 通过；与 Firefox 在当前检出上并行完整回归 147 / 147（4.1 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，73 / 73 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、单卡与多选组边界辅助线、单卡与不同尺寸选择组实时等距、抓取入口一致性、多选原子对齐 / 分布 / 模块接口混选删除、当前 Level 全选 / 清空、Copy / Paste / Duplicate / Ctrl/⌘ 拖动克隆、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、八条直接手势的边缘自动平移、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、完整障碍预览、gesture session 注册 / 复用 / 清理、pointer 候选 / 取消 / 非法 / no-op / 成功重连、键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮最终完整回归样本中首次可交互 1470 ms、模块选择 513 ms、多选两个模块 847 ms、模块搜索与定位 1588 ms、接口筛选 371 ms、接口端点聚焦 528 ms、保存并校验 78 ms；连接预览一次注册 200 obstacles，15 requests / 7 solves / 8 hits，峰值 3.5 ms、末次 0.3 ms；测得 JS + embedder + backing storage 总量 86,781,488 bytes，均为 observation-only 环境观测而非固定预算 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、1000 个 MiniMap 节点与保存保持全量，初始 Canvas DOM 只挂载视口内 200 modules / 430 connections，默认 Hierarchy 为完整 1002 行投影但首批 DOM 仅 40 行；三轮中位数：首次可交互 2552 ms，十次 Level 编辑 2692 ms、Undo 199 ms、Redo 190 ms；Sources 选择屏外模块并定位到可读尺寸 217 ms，MiniMap 首尾节点跨图定位 63 / 60 ms，四次 Hierarchy 查询、一次增量加载与最终定位 758 ms，接口筛选 131 ms、接口端点与路径定位 182 ms，保存并校验 62 ms；历史交互测得增量 18,264,378 bytes，最终 JS + embedder + backing storage 总量 65,320,494 bytes。Iteration 92 单次压力复验为首次可交互 3898 ms、十次编辑 3439 ms、Undo / Redo 229 / 211 ms、跨图选择 266 ms、多选 700 ms、guide drag 3365 ms、MiniMap 定位 73 ms、搜索 806 ms、保存 67 ms；连接预览一次注册 1000 obstacles，13 requests / 5 solves / 8 hits，峰值 6.9 ms、末次 1.0 ms，最终测量内存 83,407,759 bytes，并在取消后保留原有 2000 connections |
...
| 浏览器范围 | Chromium 74 / 74 完整自动化；Firefox 73 / 73 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
