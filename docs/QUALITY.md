...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1894 modules transformed，7.82 秒 |
| Vitest 快速单元测试 | 117 / 117 通过，18 个 test files，1.57 秒；除既有场景路由外，片段测试覆盖内部连接闭包、接口精确集合、失败原子性、一次 Undo、五层层级引用和 100 条连接 / 101 个非均匀模块的逐项重写 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，50 / 50 通过；与 Firefox 在已核验为当前检出的开发服务器上并行完整回归 99 / 99，共 3.1 分钟 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，49 / 49 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、对齐辅助线、多选原子对齐 / 分布、Copy / Paste / Duplicate、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮完整回归样本中首次可交互 1464 ms、模块选择 494 ms、多选两个模块 900 ms、四类模块搜索、一次增量加载并键盘定位 1624 ms、接口筛选 410 ms、接口端点聚焦 513 ms、保存并校验 114 ms；测得 JS + embedder + backing storage 总量 85,860,955 bytes，均为 observation-only 环境观测而非固定预算 |
...
| 编辑闭环 | 新建、模块、端口、模块移动 / 四边四角 resize、多选原子对齐 / 分布、Copy / Paste / Duplicate、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 50 / 50 完整自动化；Firefox 49 / 49 产品合同，唯一排除项为 Chromium 专属 CDP heap 采样；WebKit 尚未验证 |
