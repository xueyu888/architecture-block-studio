...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1921 modules transformed |
| Vitest 快速单元测试 | 233 / 233 通过，35 个 test files；覆盖桌面文件与最近设计服务（含并发首次写入）、五语目录完整性、场景路由、viewport auto-pan、片段、组选区、位置化模块创建、最深 Level drop-target、结构共享几何编辑 / 布局投影、canonical 历史快照、route / line-jump 帧增量、Worker 有界认证帧缓存、latest-only 请求队列、完整候选路线证书，以及真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 89 个文件、29 条唯一依赖全部带 import 证据 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，93 / 93 通过；覆盖完整 renderer 产品合同、旧浮动 Dock 自动恢复、低缩放端口真实鼠标拖动、侧栏与五语切换，以及 200 / 400 性能旅程 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，92 / 92 产品合同通过；执行除 Chromium 专属性能采样外的全部场景。Vite 开发态 inline Worker 在 Firefox 只对 localhost 根入口产生一个固定浏览器 warning，收集器以完整消息和 URL 形状精确识别；production preview 实测为 0 warning，其他 warning / error / pageerror 继续使测试失败 |
| Windows Electron | 1 / 1 通过；真实窗口覆盖 sandbox / context isolation、原生 Open / Save As / Save、最近设计跨 reload 持久化与失效清理、五语切换、固定 Dock 的隐藏 / 恢复与旧浮动布局迁移、低缩放端口抓手、dirty / Undo 和客户端更新状态 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与全部路径几何门禁。当前单次样本：首次可交互 1395 ms；全部接口 / 模块选择 498 / 543 ms；组选区 pointer release 144 ms、真实 committed ready 584 ms、Worker 28.4 ms；affected / neighborhood 6 / 27，只重投影 2 nodes / 6 edges，只回传 6 routes / 0 jump arrays；Undo 830 ms、Cut + Undo 1455 ms、保存 72 ms、总测量 114,956,144 bytes。均为 observation-only 环境观测 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、MiniMap 与保存保持全量，Canvas 只裁剪 DOM。当前单次样本：首次可交互 4245 ms、十次编辑 3459 ms；完整组选区 resize 1948 ms，其中 pointer release 127 ms、真实 committed ready 1133 ms、Worker 265.9 ms；affected / neighborhood 6 / 27，只重投影 2 nodes / 6 edges，只回传 6 routes / 0 jump arrays；Undo 1392 ms、Cut + Undo 1688 ms、保存 109 ms；1000 障碍连接预览峰值 7.7 ms；最终测量 179,680,594 bytes。Verifier 仍审计 2000 条路线和 1,999,000 个无序线对；这些是开发机 observation，不是固定预算 |
| 路由五层压力 | 双展开真实场景逐条审计 54 legs、枚举 1431 个无序线对；偏斜 fixture 让一个 5000 × 3500 Hub 拥有 100 个端口和 100 条线、100 个卫星各只有 1 条线，逐条审计 100 routes、枚举 4950 pairs，并在 Optimize Routing 后重跑；纯路由场景覆盖 120 legs / 7140 pairs；真实 UI 五层展开覆盖 20 routes / 190 pairs，并在第 4→5 层键盘重连及 Undo 后重新全量审计；源码架构五层图另覆盖 29 条真实 import 路线与 406 个无序线对，并在选择、resize / Undo、单线交叉定位、端口移动和保存前后复验；另以 12 个 commodity 穿过五层 Gate 链形成 72 legs / 2556 pairs，检查每条线的 domain 与全部线对；密集共享端口必须诚实 `Unresolved`。逐线检查有限 / 非零 / 正交 / 微短段 / 反向 / 自交，逐对检查共线、8px separation、共享 stub 豁免、每个 strict crossing 的线桥与孤立桥 |
| 几何检查 | 已覆盖模块碰撞、端点内侵、边界逃逸、设计坐标中的独立线路共路、根级兄弟重叠、线中标签、逐线与全部无序线对审计、move 的实时等距 / 边缘 / 中心吸附、不同尺寸多选组的完整包围框 / 等距 / 抓取入口一致性、组选区四边四角、Shift 等比、Alt 绕过、最小 / 最大 scale、嵌套父坐标和低缩放恒定命中区、单卡 resize 的同宽 / 同高吸附、多选六向对齐与水平 / 垂直等中心分布、Paste Here 与位置化新建的最近网格无碰撞落位、完整可见与 MiniMap / Controls 16px 安全间距、Ctrl/⌘ 拖动子图在明确指针位移处的路线穿块检查，以及大型设计 400 条路径逐线段穿块检查 |
...
| 对象菜单截图 | `object-context-menu.png` 复核 1680 × 1050 完整工作台：Agent UI 保持选中，9 个同源对象动作、快捷键、禁用原因、Inspector 与 MiniMap 清楚；底层 10 条线路仍保持正确端口方向、0 穿卡和 0 线中标签，菜单作为临时 popup 不改写任何持久几何 |
| 源码架构截图 | `source-architecture-overview.png` 复核五层边界、12 个真实模块和 29 条 import 依赖的完整总览；`source-architecture-review.png` 复核 Studio 一跳邻域与多选 Inspector；`source-architecture-interface-review.png` 复核单条依赖的完整路径、唯一 target 箭头、端口和接口证据。三图均为 1680 × 1050 真实浏览器流程，0 线中标签、0 穿卡、0 非正交、0 重复重叠、0 无桥交叉或孤立桥 |
| Windows 日常操作截图 | `windows-port-move-grip.png` 复核 36% 低缩放下独立 22 CSS px 端口移动抓手位于标签和线路之外，圆形 Handle 仍只负责连接；`windows-multilingual-recent-designs.png` 复核中文工作台、最近设计文件与目录摘要、语言选择和左右栏恢复入口，菜单不遮挡所选图形。两图均来自真实 Electron 窗口 |
| 位置化新建截图 | `add-module-here.png` 复核 toolbar drag 新模块与 MiniMap 保持安全间距；`add-module-inline-drop-target.png` 复核 31% 五层总览只高亮唯一最深目标和最终卡片，不再用截断文字胶囊遮挡原模块；`add-module-empty-level-drop-target.png` 复核 215% 空子 Level 完整容纳默认卡片预览，目标、Header、层级说明和 MiniMap 互不遮挡；`add-module-here-level-five.png` 复核新增后 3 个最深层模块及全部 20 routes / 190 pairs；`windows-add-module-here.png` 复核真实 Electron 窗口中的 toolbar drag、一次 Undo 与桌面文件工作区。五图均为最终应用渲染，0 线中标签、0 卡片 / 线路 / 端口遮挡 |
...
| 编辑闭环 | 新建、空白右键 Add Module Here、工具栏拖放模块、内联最深 Level 命中、空子 Level 第一张卡片、端口、模块移动 / 单卡与同父级多模块四边四角 resize、当前 Level 全选 / 仅模块 / 仅接口 / 清空、模块直接接口与直接邻域扩展、入站 / 出站接口与邻域扩展、多选原子对齐 / 分布 / 删除、Copy / Cut / Paste / Paste Here / Duplicate / Ctrl/⌘ 拖动完整子图克隆、选中线路完整聚焦、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、鼠标 / 键盘端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 93 / 93 完整自动化；Firefox 92 / 92 产品合同，唯一排除项为 Chromium 专属性能采样；WebKit 尚未验证，但 Windows 桌面产品不以 WebKit 或移动端为交付目标 |
...
Iteration 99 对照 draw.io 官方 [`mxPopupMenuHandler`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/mxgraph/src/handler/mxPopupMenuHandler.js#L141-L210) 的按下位置与移动容差边界，将对象菜单和画布右键平移收敛为一次互斥手势。Canvas 只记录 pointer、起点、末点与是否超过共享 5px 容差：短按按真实节点 / 路线几何产生选择意图，超容差后由 Canvas 同步更新 viewport 并永久取消菜单；Studio 继续拥有 Inspector 草稿保护与 canonical selection。`contextMenuModel` 只按模块、接口和多选组织既有 command id，`CanvasContextMenu` 只拥有位置、焦点、字符导航和关闭，名称、快捷键、可用性、禁用原因与执行仍唯一来自 `StudioCommands`。173 / 173 unit（21 files / 1.55 秒）、1902-module build（Vite 6.98 秒）和当前最终检出的 Chromium 81 / 81 + Firefox 80 / 80 = 161 / 161（7.2 分钟）完整回归通过。真实节点、线路、多选、Shift+F10、Home / End / 字符定位、Escape 恢复、草稿拒绝、右拖和 viewport 边缘约束全部验证；五层继续逐条审计 20 routes / 190 pairs，偏斜 Hub 为 100 / 4,950，200 / 400 与 1000 / 2000 压力旅程继续通过。Iteration 101 刷新的 `object-context-menu.png` 已人工复核：新增 Enter Module 后共 10 个同源对象动作，Inspector、MiniMap、底层 10 条路线、端口方向和卡片仍清楚，0 穿卡、0 线中标签。整体仍未达到 draw.io 同等级别，Iteration 100 继续用真实项目架构图总验收。

Iteration 100 把仓库自身作为真实用户输入，而不是继续增加只为测试构造的均匀图。生成器用 TypeScript AST 解析全部 65 个 `src` 文件，将每个文件唯一归入 12 个责任模块，并双向闭合 27 条跨模块相对 import；未归属、重复归属、无法解析、投影漂移和模块环都会让 `pnpm verify:self-architecture` 失败，production build 先执行同一门禁。生成 JSON 保留 product → browser runtime → composition → verified source → runtime modules 五层上下文，但第 5 层直接承载真实模块和依赖，避免用边界容器隐藏线路或破坏邻域语义。真实浏览器展开后精确得到 16 个可见块（4 个边界 + 12 个模块）、27 条可见线、0 DRC warning / error，Chromium 与 Firefox 分别逐条审计 27 routes 和全部 351 pairs；Studio 邻域、Fit Selection、卡片键盘 resize / Undo、Sources 交叉定位、物理点线、Inspector 合同、保存回读均通过。最终 175 / 175 unit（22 files / 1.70 秒）、1902-module build（Vite 7.09 秒）、Chromium 82 / 82 + Firefox 81 / 81 = 163 / 163（5.0 分钟）通过；200 / 400 全量和 1000 / 2000 视口裁剪压力旅程再次通过。三张 1680 × 1050 截图分别证明系统总览、9 modules / 8 interfaces 一跳邻域和单条 import 合同审查，均无穿卡、线中标签、非正交、重复重叠或无桥交叉。整体仍未达到 draw.io 同等级别：五层总览能保留上下文，但深入某层时仍缺少 draw.io 式进入 / 退出容器的层级聚焦，低缩放邻域文字也因此不够直接；下一轮应在不创建第二份设计事实的前提下建立可恢复的层级视图根与 breadcrumb。
...
Iteration 103 对照 draw.io 官方 [`Actions.edit`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/grapheditor/Actions.js#L705-L712) 的直接编辑入口，把模块标题从 Properties-only 收敛为可恢复的画布编辑会话。`BlockDesignDocument` 继续唯一拥有标题；BlockNode 只保存未提交字符串和原生输入焦点，Canvas 的一次性 F2 request 不进入 selection 或 JSON；Studio 经既有 Inspector 草稿守卫提交一次具名 `node/rename`，Editor 统一 trim、Schema 校验、History 和 dirty。Escape、空值和守卫拒绝均零写入；原生表单键盘先于 Canvas 对象导航，修复了 Escape 被捕获后 blur 误提交的真实浏览器问题。最终 181 / 181 unit（24 files）、1903-module build、Chromium 84 / 84 + Firefox 83 / 83 = 167 / 167（5.4 分钟）和 Electron 1 / 1 通过；五层 20 routes / 190 pairs、真实源码 27 / 351、偏斜 Hub 100 / 4,950 以及 200 / 400、1000 / 2000 压力旅程全部复验。`inline-title-editing.png` 与 `windows-inline-title-editing.png` 已人工复核：输入框只占 Header 标题槽，端口、边界、线路、Inspector 和 MiniMap 无遮挡，线中标签仍为 0。整体仍未达到 draw.io，下一轮继续位置化插入 / 粘贴与桌面手势复评。

Iteration 104 对照 draw.io 官方 [`Actions.pasteHere`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/grapheditor/Actions.js#L200-L208)、[`EditorUi.pasteFromClipboard(point)`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/diagramly/EditorUi.js#L20695-L20767) 与 [`Graph.moveCellsTo`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/grapheditor/Graph.js#L12140-L12170)，把“右键在哪里”从菜单位置收敛为显式但可丢弃的 Level 设计坐标。Canvas 从任一已渲染同层模块的 `positionAbsolute - designPosition` 反推父容器投影原点，因此第 1 层和第 5 层使用同一换算；`fragmentPlacement` 把片段 bounds 左上角对齐 32px 网格，被占用时按最近确定性候选避让；Studio 的一个 `insertFragment` 再组合 cascade、point 或 explicit offset 三类 placement policy，片段重写、Editor、History、selection 与 reveal 不分叉。最终 183 / 183 unit（24 files）、1903-module build、Chromium 85 / 85 + Firefox 84 / 84 = 169 / 169（5.5 分钟）和 Electron 1 / 1 通过；真实空白画布 Paste Here 的下载 JSON 精确等于目标网格坐标，第 5 层对象附近碰撞避让后仍审计 20 routes / 190 pairs，完整回归继续覆盖源码 27 / 351、Hub 100 / 4,950、200 / 400 与 1000 / 2000。即时对比度门禁还发现 context menu opacity 动画令 `Ctrl/⌘ A` 首帧只有 3.42:1，现只保留 2px 位移并达到 4.5:1。`paste-here.png`、`paste-here-level-five.png` 与 `windows-paste-here.png` 已人工复核：目标模块、五层边界、原线路、端口、Inspector 和 MiniMap 清楚，0 线中标签、0 卡片或线路遮挡。整体仍未达到 draw.io，下一轮继续位置化新建模块与桌面拖放。

Iteration 105 对照 draw.io 官方 [`Sidebar.createDropHandler`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/grapheditor/Sidebar.js#L3742-L3908) 与 [`Sidebar.addClickHandler`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/grapheditor/Sidebar.js#L5533-L5606)，把右键 Add Module Here 与 Windows 工具栏拖放收敛到同一个位置化模块创建合同。Canvas 只把 pointer 投影为当前 view root 的 Level 设计坐标并显示可丢弃虚线预览；Dialog 仍只拥有未提交名称 / Owner；Studio 在确认后才用真实卡片尺寸、当前 Level 全部 authored rect 和 `findBlockPlacementAtPoint` 求 32px 网格上的最近无碰撞位置；Editor 最终只执行一次带 `position / pinned` 的 `node/add`。普通 Add Module 仍选择自动 policy，不分叉创建、Schema、History 或 dirty。截图人工复核还发现新卡片虽在画布内，却会被 MiniMap 部分遮住；最终将 reveal 条件收紧为完整可见且与所有 viewport obstruction 保持 16 CSS px，已可见时则不制造视口跳动。最终 185 / 185 unit（24 files）、1904-module build、Chromium 86 / 86 + Firefox 85 / 85 = 171 / 171 和 Electron 1 / 1 通过；真实右键、toolbar drag、精确下载 JSON、取消 / 草稿拒绝、一次 Undo、五层碰撞避让均成立，第 5 层新增后仍逐条审计 20 routes / 190 pairs，源码 27 / 351、Hub 100 / 4,950、200 / 400 与 1000 / 2000 继续全量覆盖。`add-module-here.png`、`add-module-here-level-five.png` 与 `windows-add-module-here.png` 已逐张人工复核，0 线中标签、0 卡片 / 线路 / 端口 / MiniMap 遮挡。整体仍未达到 draw.io：当前 toolbar drop 明确落入 current view root，但 inline 展开的子 Level 尚无直接 drop target 高亮，下一轮继续收敛层级目标识别。

Iteration 106 对照 draw.io 官方 [`mxDragSource.dragOver / mouseUp`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/mxgraph/src/util/mxDragSource.js#L628-L706) 与 [`Sidebar.createDropHandler`](https://github.com/jgraph/drawio/blob/dev/src/main/webapp/js/grapheditor/Sidebar.js#L3742-L3899)，把内联层级命中从 Canvas 猜测收敛为 Layout 发布的显式投影。每个展开 owner 只发布 child Level 的 id、title、depth、design origin 和 visible drop bounds；空子 Level 使用默认模块尺寸建立真实表面。纯 `moduleDropTarget` 只选择指针下最深候选；Canvas 只投影唯一蓝色轮廓和由 `findBlockPlacementAtPoint` 返回的最终尺寸预览，删除会在 31% 缩放下截断并遮挡内容的目标文字胶囊。drop 与 Dialog 确认仍复用同一 placement / `node/add` 链，最终位置以用户实际看到的预览为浏览器合同，不把 CSS 像素舍入写成固定业务坐标。最终 191 / 191 unit（25 files）、1905-module build、Chromium 87 / 87 + Firefox 86 / 86 = 173 / 173 和 Electron 1 / 1 通过；五层拖放另以 6 个并发 worker 重复 6 / 6，空层下载 JSON 精确为 `{x:0,y:0,pinned:true}`，Undo 后重新为空；第 5 层新增后仍逐条审计 20 routes / 190 pairs，Hub 100 / 4,950、源码 27 / 351、200 / 400 与既有 1000 / 2000 压力门禁保持覆盖。两张新增截图逐图人工复核，0 文字胶囊、0 卡片 / 线路 / 端口 / MiniMap 遮挡。整体仍未达到 draw.io：当目标 Level 自身还包含已展开后代时，当前自动投影可能在提交后重新排列其直接模块；Iteration 107 必须逐层验证并统一 authored drop 与最终可见位置。

Iteration 107 对照 draw.io 官方 [`Sidebar.createDropHandler`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/js/grapheditor/Sidebar.js#L3742-L3908)、[`mxGraph.cellsAdded`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/view/mxGraph.js#L4835-L4954) 和默认返回 `null` 的 [`mxLayoutManager.getLayout`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/view/mxLayoutManager.js#L227-L291)，移除了“Level 只要存在展开 child 就自动 ELK 排列”的隐式规则。JSON `node.layout.position` 继续唯一拥有 authored 坐标；Layout 新增只读 `projectedPosition`，以文档顺序、32px 网格和正方向追加稳定搜索为展开容器建立无碰撞投影，已有模块不会因后来插入而移动。普通 Level 使用 24px 净空，含可见展开后代的 Level 使用自身 spacing + 24px；负坐标子内容只改变 owner 外框包围，不修改保存坐标。Studio 的 drop preview、Dialog confirm 与最终 `node/add` 共用同一 gap 和内联最小原点 policy，根画布仍允许无限坐标。最终 195 / 195 unit（25 files）、1905-module production build、Chromium 88 / 88 + Firefox 87 / 87 = 175 / 175（8.2 分钟）和 Electron Windows 壳 1 / 1 通过；五层真实拖放还以 6 个并发 Chromium worker 重复 6 / 6。该旅程逐层验证预览设计矩形、viewport、最终 client bounds、保存 JSON 和一次 Undo；完成后 20 routes / 190 pairs 逐条与全对审计均为 0 几何问题。双展开默认图 54 / 1,431、源码图 27 / 351、偏斜 Hub 100 / 4,950、200 / 400 与 1000 / 2000 继续由全量回归覆盖。`add-module-five-depth-contract.png`、`add-module-five-depth-detail.png` 与三张双展开细节图已逐图人工复核：入口方向、端口文字和独立槽道清楚，0 线中标签、0 穿卡、0 卡片 / 端口 / MiniMap 遮挡。整体仍未达到 draw.io；下一轮要把内联 Level 的同一最小边界扩展到已有模块的直接 move / resize，避免把模块向当前投影原点之外拖动时导致容器归一化和兄弟投影位移。

Iteration 108 对照 draw.io 的 [`transparentBounds`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/js/grapheditor/Graph.js#L12439-L12449)、[`constrainChildren / extendParents`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/view/mxGraph.js#L1368-L1414) 与 [`extendParent`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/view/mxGraph.js#L6283-L6326)，把“父容器外框”和“子 Level 坐标原点”拆成两个事实。Layout 只发布稳定 `coordinateOrigin = min(0, authoredMin)`；非负 Level 永远以 `(0,0)` 为原点，新增或移动一个模块不会再把未参与手势的兄弟整体平移。纯 `layout/levelGeometry` 统一求 pointer、Alt、键盘、单体 resize 与组选区 resize 的合法 delta / start edge；Alt 只跳过网格和参考线，不能越过 Level 边界；负坐标旧文件若由当前对象拥有最小原点，则锁住该轴，避免静默改写整个坐标系。组 resize 提交改为用 `designPosition + previewDelta` 回到 JSON，不再把 React Flow 的 parent-local padding 当成 authored 坐标；右 / 下扩展只让临时父框增长，最终外框仍从 JSON 重建。路由器同时把可见图 arc、审美 tie-break 与验证证书的几何签名改为相对 source 的平移不变量，完整场景平移不再改变等价线路选择。最终 201 / 201 unit（26 files）、1906-module production build、Chromium 88 / 88（7.2 分钟）+ Firefox 87 / 87（5.3 分钟）= 175 / 175，Electron Windows 壳 1 / 1 通过；12 模块 / 69 源文件 / 27 跨模块依赖 / 五层生成门禁通过。五层 20 routes / 190 pairs、双展开 54 / 1,431、源码 27 / 351、偏斜 Hub 100 / 4,950、200 / 400 与 1000 / 2000 均执行完整路线或压力旅程。刷新八张全图 / 局部截图并新增 `five-level-coordinate-resize.png`；逐图人工复核确认方向、端口名、组选区、独立槽道清楚，0 线中标签、0 穿卡、0 无意义回钩、0 卡片或端口遮挡。整体仍未达到 draw.io；下一轮继续验证直接操作向右 / 下越过当前外框时的父容器增长、视口保持和高连接度增量路由。

Iteration 109 对照 draw.io 当前固定源码中 [`mxGraphHandler.updateLivePreview`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/handler/mxGraphHandler.js#L1454-L1615) 对临时节点状态与相连 Edge 的同步刷新，以及 [`mxGraph.extendParent`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/view/mxGraph.js#L6283-L6326) 只向右 / 下扩展父级的边界，消除了“节点使用 live geometry、Router 仍使用 committed geometry”的双帧根因。`BLOCK_CONTAINER_GEOMETRY` 唯一拥有容器 padding；`projectCompoundNodeGrowth` 从最深 owner 向外递归五层且不移动任何原点；Canvas 的 disposable scene frame 同时投影节点、端口、障碍、Gate 和路线。至多 32 条受影响 leg 直接精确求解；大图按端点 / 域 / locked geometry / 障碍相交、Gate 与真实容量邻域闭包后交给 latest-only Worker，过期结果不上屏，失败则保留已验证 committed routes；pointerup 的 settling 必须等新文档帧与 live frame 签名相同再释放，线路不再先回旧位置。最终 207 / 207 unit（28 files）、1909-module production build、Chromium 89 / 89（5.4 分钟）+ Firefox 88 / 88（5.7 分钟）= 177 / 177，Electron Windows 壳 1 / 1 通过；12 模块 / 74 源文件 / 27 跨模块依赖 / 五层生成门禁通过。五层 live growth 逐条审计 20 routes / 190 pairs，并验证五个父框只向右 / 下增长、viewport 不跳、松手前后 route points 相同、只有最深目标的 authored geometry 改变；非均匀 100 线 Hub 在两片叶子仍按住 resize 时审计 100 / 4,950；200 / 400 live resize 审计 400 / 79,800，受影响 6 条、容量邻域 27 条、Worker 12.6 ms。`five-level-live-routing-growth.png` 与 `routing-stress-live-preview.png` 已人工复核：入口方向、端口文字、线路槽道和选择轮廓清楚，0 线中标签、0 穿卡、0 旧端点回钩。整体仍未达到 draw.io：1000 / 2000 本机实时 Worker 求路约 44.9 ms，但整次组 resize / Undo 仍约 5.4 / 3.0 秒，下一轮必须把松手后的 committed routing 与 1000 节点受控投影移出主线程长任务。

Iteration 110 对照同一固定 draw.io 源码的 [`mxGraphHandler.updateLivePreview`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/handler/mxGraphHandler.js#L1459-L1615) 局部 invalidation 与 [`mxGraphModel.beginUpdate / endUpdate`](https://github.com/jgraph/drawio/blob/a1f615b7f5a5237da71de2ce2f057b5fa70b0aeb/src/main/webapp/mxgraph/src/model/mxGraphModel.js#L1964-L2019) 最外层单次发布，将 committed route 收敛为版本配对、可丢弃计算、完整验证后一次安装的帧。`reconcileLayoutResult` 只复用完整投影相等的 layout item；live / committed 共用一个 inline Worker 与判别协议，每个 hook 的 desired / queued / in-flight 队列保证运行期间只保留最新请求。普通几何编辑只重算 affected neighborhood，再对完整候选逐 leg / pair 认证；Optimize Routes 才强制 full solve。过期或失败保留上一认证帧，route、jump、node 与 edge 引用只替换真实变化项。整帧 `RoutingResult.status` 从 Edge 收回 Canvas，修复了状态变化导致 400 / 2000 条线全重建；Fit 同样改为一次性命令，新增 / 删除节点不再重放历史 Fit。最终 214 / 214 unit（31 files）、1915-module build、Chromium 89 / 89、Firefox 全部产品场景和 Electron 真实窗口通过；Windows 连续 5 / 5 额外验证初始 Fit、F2、Enter、Undo、Copy、Paste Here、toolbar drop 与原生文件链。200 / 400 的 committed Worker / ready 为 26.1 / 711 ms，1000 / 2000 为 246.7 / 1343 ms；两档均只投影 2 nodes / 6 edges，affected / neighborhood 为 6 / 27。刷新后的 Windows 三张截图已人工复核：37% 初始 Fit、标题输入、粘贴与模块新建都保持完整工作台，0 线中标签、0 卡片 / 端口 / 面板遮挡。整体仍未达到 draw.io：压力档完整组选区 resize 约 3.2 秒，下一轮继续用主线程 trace 缩短 gesture 到认证帧的可见延迟。

Iteration 111 用 Chromium CPU profile 把大图 pointerup 后的长尾分解到 Editor / History、Layout、Worker structured clone、Canvas traversal 与 React Flow 提交，而不是放宽 verifier 或减少设计事实。几何 move / resize 现在由 Editor 对完整批次和变更节点局部校验，并结构共享未变 Level、节点、连接和接口；History 原子维护当前 canonical snapshot，dirty / save / Undo / Redo 不再反复序列化同一文档；平面 authored 场景只把变更节点投影到既有 Layout，有展开层级或 automatic placement 时自动回退完整 composer。Worker 有界保存 4 个认证帧，浏览器只发送当前 `RoutingScene + previousFrameKey`；返回结果以精确 base key 携带 route / jump map patch，主线程只有拥有同一认证 base 才重建完整结果。Canvas traversal 的同父级 / 同 Level 分组同时从平方级数组复制改成线性构建。完整 verifier 仍逐条审计全部路线和所有无序线对。最终 218 / 218 unit（32 files）、1917-module build、Chromium 89 / 89 + Firefox 88 / 88 = 177 / 177、Electron Windows 壳 1 / 1 全部通过。200 / 400 只回传 6 routes / 0 jumps，committed ready 584 ms；1000 / 2000 完整组选区 resize 从 3222 ms 降到 1948 ms，pointer release 127 ms、committed ready 1133 ms、Worker 265.9 ms，同样只回传 6 / 0 并保持 2 nodes / 6 edges 重投影。刷新后的百线枢纽、五层总览 / 局部、五级父框增长、组选区缩放与四张 Windows 截图均已逐张人工复核：箭头方向、端口文字和独立槽道可辨，0 线中标签、0 穿卡、0 卡片 / 端口 / 面板遮挡。整体仍未达到 draw.io：压力场景的完整认证与 React Flow / DOM 收敛仍有约 1.1 秒后台长尾，作为本目标收尾后的明确保留项。

Iteration 112 把语言、Dock、端口手势和最近文件拆成四个正交事实域。`StudioLocaleProvider` 以一个类型完整的五语目录派生核心工作台文案，本机偏好不进入 JSON；Dockview 继续唯一持有左右栏显隐，并始终保留恢复 rail。端口的 `side + offset` 仍只由文档拥有，外侧 inverse-zoom 抓手只生成 pointer intent，圆形 Handle 继续只负责连接，Canvas 预览和相关线路在一次 `port/move` 前均可丢弃。Electron main 的 `RecentDesignService` 以串行事务队列原子维护最多 10 个文件引用；renderer 只使用 opaque id，最近记录不复制设计 JSON。最终 233 / 233 unit（35 files）、1921-module production build、Chromium 92 / 92 + Firefox 91 / 91 = 183 / 183、Electron Windows 壳 1 / 1 全部通过。并发首次最近记录、重载持久化、去重、顺序、失效清理，五语切换不改文档，侧栏 Header / rail 往返，以及 36% 缩放真实鼠标跨边拖端口、路线跟随、dirty、Undo 均有自动化证据；29 条真实源码路线 / 406 pairs、100 线 Hub / 4,950 pairs 和 1000 / 2000 压力旅程继续完整通过。两张新增 Windows 截图已人工复核抓手、标签、线路、最近菜单、左右 rail 和 Inspector 无遮挡。

Iteration 113 将 Dock 从“可任意浮动的通用容器”收敛为产品明确需要的固定工作台。`createDefaultLayout` 是 Sources / Canvas / Properties / Messages 面板拓扑的唯一 Owner；localStorage 只保存尺寸、折叠与激活状态。恢复时先比较完整 panel set 以及 center、四边、floating、popout placement，异常布局统一恢复并重写 canonical layout；`disableDnd` 与删除 Float 入口阻止新异常状态。最终 233 / 233 unit（35 files）、1921-module production build、Chromium 93 / 93 + Firefox 92 / 92 = 185 / 185、Electron Windows 壳 1 / 1 全部通过。浏览器与 Windows 真实窗口均构造旧版浮动 Properties，刷新后验证中央无浮动面板、Properties 回到右栏、持久布局不再含 floating group；`windows-fixed-properties-dock.png` 已人工复核画布、右栏、模块、端口与线路无遮挡。
