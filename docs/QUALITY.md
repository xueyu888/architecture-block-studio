...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1894 modules transformed，7.30 秒 |
| Vitest 快速单元测试 | 119 / 119 通过，18 个 test files，1.55 秒；除既有场景路由与片段合同外，选择测试覆盖当前 Level 全量 canonical node / connection 集合与空 Level，viewport framing 测试覆盖模块矩形、完整线路点集、退化点和空输入 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，54 / 54 通过；与 Firefox 在已核验为当前检出的开发服务器上并行完整回归 107 / 107，共 3.1 分钟 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，53 / 53 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、对齐辅助线、多选原子对齐 / 分布、当前 Level 全选 / 清空、Copy / Paste / Duplicate、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮完整回归样本中首次可交互 1489 ms、模块选择 512 ms、多选两个模块 1000 ms、四类模块搜索、一次增量加载并键盘定位 1668 ms、接口筛选 434 ms、接口端点聚焦 577 ms、保存并校验 100 ms；测得 JS + embedder + backing storage 总量 86,218,426 bytes，均为 observation-only 环境观测而非固定预算 |
...
| 编辑闭环 | 新建、模块、端口、模块移动 / 四边四角 resize、当前 Level 全选 / 清空、多选原子对齐 / 分布、Copy / Paste / Duplicate、选中线路完整聚焦、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 54 / 54 完整自动化；Firefox 53 / 53 产品合同，唯一排除项为 Chromium 专属 CDP heap 采样；WebKit 尚未验证 |
...
Iteration 66 对照 draw.io 的 vertex handler 边界，将“内容安全尺寸”“gesture 预览”和“持久几何提交”拆开：`layout/nodeGeometry` 计算四侧端口与内容共同要求的最小尺寸，选中模块显示 4 个角控制点和 4 条边命中带；视觉点保持轻量，18 CSS px 命中区在缩放后仍可操作，Port rail 位于其上避免被遮挡。pointer 松手只提交一次原子 `node/resize`，同时保存 position、width、height 与 pinned；Shift + Arrow 以 16 设计像素调整宽高。真实双浏览器旅程证明 resize 后线路跟随、下载 JSON、Undo / Redo、键盘焦点和 live 公告一致；未应用 Inspector 草稿会拒绝提交并恢复原尺寸。88 / 88 unit（0.457 秒）、1883-module build（6.87 秒）、完整 Chromium 41 / 41 + Firefox 40 / 40 = 81 / 81（2.3 分钟）通过；`node-resize.png` 人工复核抓手、端口、线路与 Inspector 尺寸无遮挡。
