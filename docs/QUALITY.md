...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1902 modules transformed，Vite 7.09 秒 |
| Vitest 快速单元测试 | 175 / 175 通过，22 个 test files / 1.70 秒；除既有场景路由、viewport auto-pan、片段、组选区与 Editor 原子合同外，新增真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 65 个文件、27 条唯一依赖全部带 import 证据 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，82 / 82 通过；与 Firefox 在当前最终检出上并行完整回归 163 / 163（5.0 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，81 / 81 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，并与 Chromium 共同加载五层源码架构示例、展开 12 个真实模块、逐条审查 27 条 import 依赖及全部 351 个无序线对；其余覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、组选区八向 resize、对齐 / 等距、片段操作、右键菜单、路由编辑、连接预览、五层与 Hub 压力、命令入口和保存导出 |
...
| 浏览器范围 | Chromium 82 / 82 完整自动化；Firefox 81 / 81 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
