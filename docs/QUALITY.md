...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，92 / 92 通过；覆盖完整 renderer 产品合同、低缩放端口真实鼠标拖动、侧栏与五语切换，以及 200 / 400 性能旅程 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，91 / 91 产品合同通过；执行除 Chromium 专属性能采样外的全部场景。Vite 开发态 inline Worker 在 Firefox 只对 localhost 根入口产生一个固定浏览器 warning，收集器以完整消息和 URL 形状精确识别；production preview 实测为 0 warning，其他 warning / error / pageerror 继续使测试失败 |
| Windows Electron | 1 / 1 通过；真实窗口覆盖 sandbox / context isolation、原生 Open / Save As / Save、最近设计跨 reload 持久化与失效清理、五语切换、左右侧栏显隐、低缩放端口抓手、dirty / Undo 和客户端更新状态 |
...
| 浏览器范围 | Chromium 92 / 92 完整自动化；Firefox 91 / 91 产品合同，唯一排除项为 Chromium 专属性能采样；WebKit 尚未验证，但 Windows 桌面产品不以 WebKit 或移动端为交付目标 |
