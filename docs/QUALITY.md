...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1894 modules transformed，8.52 秒 |
| Vitest 快速单元测试 | 128 / 128 通过，18 个 test files；除既有场景路由与片段合同外，端点测试覆盖 source / target 候选、当前连接解析、替代配对与非法方向，选择测试覆盖 Connection 上下文解析以及当前 Level 全量 canonical node / connection 集合 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，60 / 60 通过；与 Firefox 在当前检出的独立开发服务器上并行完整回归 119 / 119 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，59 / 59 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、对齐辅助线、多选原子对齐 / 分布、当前 Level 全选 / 清空、Copy / Paste / Duplicate / Ctrl/⌘ 拖动克隆、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、鼠标 / 键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 浏览器范围 | Chromium 60 / 60 完整自动化；Firefox 59 / 59 产品合同，唯一排除项为 Chromium 专属 CDP heap 采样；WebKit 尚未验证 |
