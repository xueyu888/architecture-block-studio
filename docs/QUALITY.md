...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1904 modules transformed |
| Vitest 快速单元测试 | 185 / 185 通过，24 个 test files；覆盖桌面文件服务、场景路由、viewport auto-pan、片段、组选区、位置化模块创建、包括具名 `node/rename` / `node/add` 在内的 Editor 原子合同与真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 67 个文件、27 条唯一依赖全部带 import 证据 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，86 / 86 通过；与 Firefox 在当前最终检出上并行完整回归 171 / 171 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，85 / 85 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，并与 Chromium 共同验证画布指定位置新建、真实 toolbar drag、五层源码架构、12 个真实模块、27 条 import 依赖及全部 351 个无序线对；其余覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、组选区八向 resize、对齐 / 等距、片段操作、右键菜单、路由编辑、连接预览、五层与 Hub 压力、命令入口和保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮最终完整回归样本中首次可交互 1418 ms、选择全部 400 interfaces / 200 modules 为 747 / 513 ms、模块选择 434 ms、对象菜单 201 ms、直接邻域 / 出站邻域为 702 / 713 ms、多选两个模块 700 ms、两模块成组 resize / Undo 为 1376 / 930 ms、Alt 命中 218 ms、键盘遍历 398 ms、模块搜索与定位 1767 ms、接口筛选 429 ms、接口端点聚焦 501 ms、单模块 Cut + Undo 1672 ms、保存并校验 76 ms；连接预览峰值 4.4 ms、末次 0.4 ms；测得总量 110,955,894 bytes，均为 observation-only 环境观测而非固定预算 |
...
| 位置化新建截图 | `add-module-here.png` 复核 toolbar drag 新模块与 MiniMap 保持安全间距，原 10 条线、端口、选择、Properties 和反馈清楚；`add-module-here-level-five.png` 复核五层展开、3 个最深层模块及全部 20 routes / 190 pairs；`windows-add-module-here.png` 复核真实 Electron 窗口中的 toolbar drag、一次 Undo 与桌面文件工作区。三图均为最终应用渲染，0 线中标签、0 卡片 / 线路 / 端口遮挡 |
...
| 编辑闭环 | 新建、空白右键 Add Module Here、工具栏拖放模块、端口、模块移动 / 单卡与同父级多模块四边四角 resize、当前 Level 全选 / 仅模块 / 仅接口 / 清空、模块直接接口与直接邻域扩展、入站 / 出站接口与邻域扩展、多选原子对齐 / 分布 / 删除、Copy / Cut / Paste / Paste Here / Duplicate / Ctrl/⌘ 拖动完整子图克隆、选中线路完整聚焦、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、鼠标 / 键盘端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 86 / 86 完整自动化；Firefox 85 / 85 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证，但 Windows 桌面产品不以 WebKit 或移动端为交付目标 |
