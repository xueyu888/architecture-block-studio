...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1899 modules transformed，6.80 秒 |
| Vitest 快速单元测试 | 158 / 158 通过，19 个 test files / 1.48 秒；除既有场景路由、片段、viewport auto-pan 和 Editor 原子合同外，覆盖模型直接邻接、共享接口去重、自环、五层跨 Level 选择扩展及空 / 单类型 / 完整类型选择边界 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，76 / 76 通过；与 Firefox 在当前检出上并行完整回归 151 / 151（4.3 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，75 / 75 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、单卡与多选组边界辅助线、单卡与不同尺寸选择组实时等距、抓取入口一致性、多选原子对齐 / 分布 / 模块接口混选删除、从模块扩展 1 / 8 / 100 条直接接口、当前 Level 全选 / 清空 / 按模块或接口类型选择、Copy / Paste / Duplicate / Ctrl/⌘ 拖动克隆、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、八条直接手势的边缘自动平移、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、完整障碍预览、gesture session 注册 / 复用 / 清理、pointer 候选 / 取消 / 非法 / no-op / 成功重连、键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮最终完整回归样本中首次可交互 1465 ms、选择全部 400 interfaces / 200 modules 为 711 / 593 ms、模块选择 470 ms、多选两个模块 765 ms、模块搜索与定位 1623 ms、接口筛选 456 ms、接口端点聚焦 511 ms、保存并校验 97 ms；连接预览一次注册 200 obstacles，15 requests / 7 solves / 8 hits，峰值 3.6 ms、末次 0.3 ms；测得 JS + embedder + backing storage 总量 92,696,335 bytes，均为 observation-only 环境观测而非固定预算 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、1000 个 MiniMap 节点与保存保持全量，初始 Canvas DOM 只挂载视口内 200 modules / 430 connections，默认 Hierarchy 为完整 1002 行投影但首批 DOM 仅 40 行；三轮中位数：首次可交互 2552 ms，十次 Level 编辑 2692 ms、Undo 199 ms、Redo 190 ms；Sources 选择屏外模块并定位到可读尺寸 217 ms，MiniMap 首尾节点跨图定位 63 / 60 ms，四次 Hierarchy 查询、一次增量加载与最终定位 758 ms，接口筛选 131 ms、接口端点与路径定位 182 ms，保存并校验 62 ms；历史交互测得增量 18,264,378 bytes，最终 JS + embedder + backing storage 总量 65,320,494 bytes。Iteration 94 单次压力复验为首次可交互 4129 ms、选择全部 2000 interfaces / 1000 modules 为 1167 / 626 ms、十次编辑 3437 ms、Undo / Redo 228 / 214 ms、跨图选择 254 ms、多选 850 ms、guide drag 3399 ms、MiniMap 定位 66 ms、搜索 796 ms、接口筛选 / 定位 136 / 199 ms、保存 69 ms；连接预览一次注册 1000 obstacles，13 requests / 6 solves / 7 hits，峰值 7.3 ms、末次 0.9 ms，最终测量内存 90,098,327 bytes，并在取消后保留原有 2000 connections |
...
| 几何检查 | 已覆盖模块碰撞、端点内侵、边界逃逸、设计坐标中的独立线路共路、根级兄弟重叠、线中标签、逐线与全部无序线对审计、move 的实时等距 / 边缘 / 中心吸附、不同尺寸多选组的完整包围框 / 等距 / 抓取入口一致性、等距候选的正交轴重叠与优先级、resize 的同宽 / 同高吸附与 Alt 单次绕过、多选六向对齐与水平 / 垂直等中心分布、粘贴子图最近网格无碰撞落位、Ctrl/⌘ 拖动子图在明确指针位移处的路线穿块检查，以及大型设计 400 条路径逐线段穿块检查 |
...
| 编辑闭环 | 新建、模块、端口、模块移动 / 四边四角 resize、当前 Level 全选 / 仅模块 / 仅接口 / 清空、模块直接接口扩展、多选原子对齐 / 分布 / 删除、Copy / Paste / Duplicate / Ctrl/⌘ 拖动完整子图克隆、选中线路完整聚焦、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、鼠标 / 键盘端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 76 / 76 完整自动化；Firefox 75 / 75 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
