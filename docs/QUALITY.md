...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1903 modules transformed，Vite 7.29 秒 |
| Vitest 快速单元测试 | 181 / 181 通过，24 个 test files；覆盖桌面文件服务、场景路由、viewport auto-pan、片段、组选区、包括具名 `node/rename` 在内的 Editor 原子合同与真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 66 个文件、27 条唯一依赖全部带 import 证据 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，84 / 84 通过；与 Firefox 在当前最终检出上并行完整回归 167 / 167（5.4 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，83 / 83 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，并与 Chromium 共同验证画布标题直接编辑、五层源码架构、12 个真实模块、27 条 import 依赖及全部 351 个无序线对；其余覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize、组选区八向 resize、对齐 / 等距、片段操作、右键菜单、路由编辑、连接预览、五层与 Hub 压力、命令入口和保存导出 |
...
| 浏览器范围 | Chromium 84 / 84 完整自动化；Firefox 83 / 83 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
