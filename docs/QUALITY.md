...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1897 modules transformed，6.89 秒 |
| Vitest 快速单元测试 | 135 / 135 通过，18 个 test files / 1.52 秒；除既有场景路由与片段合同外，覆盖预览 / 提交端点一致性、gesture session 重复请求命中、变化坐标即时求解、30ms 失效、销毁隔离、障碍目录空间查询与外来目录拒绝 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，62 / 62 通过；与 Firefox 在当前检出上并行完整回归 123 / 123（3.4 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，61 / 61 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、对齐辅助线、多选原子对齐 / 分布、当前 Level 全选 / 清空、Copy / Paste / Duplicate / Ctrl/⌘ 拖动克隆、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、完整障碍预览、gesture session 注册 / 复用 / 清理、pointer 候选 / 取消 / 非法 / no-op / 成功重连、键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 浏览器范围 | Chromium 62 / 62 完整自动化；Firefox 61 / 61 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
