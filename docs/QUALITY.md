...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1902 modules transformed，Vite 6.98 秒 |
| Vitest 快速单元测试 | 173 / 173 通过，21 个 test files / 1.55 秒；除既有场景路由、viewport auto-pan、片段、组选区与 Editor 原子合同外，覆盖共享 5px 右键手势容差、右键命中时的 canonical 选择保持 / 替换、模块 / 接口 / 多选命令分组、accessible name 和菜单四边 viewport 约束 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，81 / 81 通过；与 Firefox 在当前最终检出上并行完整回归 161 / 161（7.2 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，80 / 80 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、不同尺寸组选区八向 resize、单卡与多选组边界辅助线、单卡与不同尺寸选择组实时等距、抓取入口一致性、多选原子对齐 / 分布 / 模块接口混选删除、从模块扩展 1 / 8 / 100 条直接接口、按 source / target 选择入站 / 出站接口与邻域、当前 Level 全选 / 清空 / 按模块或接口类型选择、Copy / Cut / Paste / Duplicate / Ctrl/⌘ 拖动克隆、模块 / 接口 / 多选右键菜单、Shift+F10、草稿拒绝和右拖互斥、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、八条直接手势的边缘自动平移、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、完整障碍预览、gesture session 注册 / 复用 / 清理、pointer 候选 / 取消 / 非法 / no-op / 成功重连、键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 大型设计 | 200 modules / 400 connections 保持全量 Canvas DOM 与 400 条路径逐线段几何检查，默认 Hierarchy 首批仅挂载 40 行；本轮最终完整回归样本中首次可交互 1464 ms、选择全部 400 interfaces / 200 modules 为 795 / 496 ms、模块选择 444 ms、对象菜单 202 ms、直接邻域 / 出站邻域为 635 / 734 ms、多选两个模块 694 ms、两模块成组 resize / Undo 为 1395 / 883 ms、Alt 命中 233 ms、键盘遍历 405 ms、模块搜索与定位 1677 ms、接口筛选 495 ms、接口端点聚焦 524 ms、单模块 Cut + Undo 1623 ms、保存并校验 76 ms；连接预览峰值 4.3 ms、末次 0.4 ms；测得 JS + embedder + backing storage 总量 109,731,159 bytes，均为 observation-only 环境观测而非固定预算 |
| 压力设计 | 1000 modules / 2000 connections 的模型、布局、1000 个 MiniMap 节点与保存保持全量，初始 Canvas DOM 只挂载视口内 200 modules / 430 connections，默认 Hierarchy 为完整 1002 行投影但首批 DOM 仅 40 行；三轮中位数仍是历史趋势证据。本轮 Iteration 99 单次复验为首次可交互 4135 ms、选择全部 2000 interfaces / 1000 modules 为 1134 / 678 ms、十次编辑 3566 ms、对象菜单 96 ms、直接邻域 / 出站邻域为 763 / 667 ms、多选两个模块 600 ms、两模块成组 resize / Undo 为 3343 / 2757 ms、单模块 Cut + Undo 5799 ms、保存并校验 673 ms，最终测量内存 183,530,131 bytes；这些是本机 observation，不是固定性能预算 |
...
| 浏览器范围 | Chromium 81 / 81 完整自动化；Firefox 80 / 80 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
