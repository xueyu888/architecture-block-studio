...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过，1899 modules transformed，7.03 秒 |
| Vitest 快速单元测试 | 161 / 161 通过，19 个 test files / 1.53 秒；除既有场景路由、片段、viewport auto-pan 和 Editor 原子合同外，覆盖模型直接邻接的 both / incoming / outgoing、共享接口去重、自环、五层跨 Level 选择扩展及空 / 单类型 / 完整类型选择边界 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium build 1234，78 / 78 通过；与 Firefox 在当前检出上并行完整回归 155 / 155（7.3 分钟） |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，77 / 77 通过；执行除 Chromium CDP heap 压力采样外的全部产品合同，覆盖文件、编辑、审查、可访问性、焦点、Dock、层级、模块 move / resize / Shift 比例 resize、单卡与多选组边界辅助线、单卡与不同尺寸选择组实时等距、抓取入口一致性、多选原子对齐 / 分布 / 模块接口混选删除、从模块扩展 1 / 8 / 100 条直接接口、按 source / target 选择入站 / 出站接口与邻域、当前 Level 全选 / 清空 / 按模块或接口类型选择、Copy / Paste / Duplicate / Ctrl/⌘ 拖动克隆、选中线路完整聚焦、Zoom / Actual Size、Space / middle / right / wheel pan、八条直接手势的边缘自动平移、场景级自动路由、五层真实展开、100 连接偏斜压力、路线线段 / 折点编辑、完整障碍预览、gesture session 注册 / 复用 / 清理、pointer 候选 / 取消 / 非法 / no-op / 成功重连、键盘端点重连、方向 marker、四侧标签轨道、Tooltip、Command Palette 与保存导出 |
...
| 编辑闭环 | 新建、模块、端口、模块移动 / 四边四角 resize、当前 Level 全选 / 仅模块 / 仅接口 / 清空、模块直接接口与直接邻域扩展、入站 / 出站接口与邻域扩展、多选原子对齐 / 分布 / 删除、Copy / Paste / Duplicate / Ctrl/⌘ 拖动完整子图克隆、选中线路完整聚焦、移动 / 尺寸辅助线与 Alt 绕过、鼠标拖线、键盘端点选择、手动路由、鼠标 / 键盘端点重连、层级绑定、Undo / Redo、Save / Save As / Export 已覆盖 |
| 浏览器范围 | Chromium 78 / 78 完整自动化；Firefox 77 / 77 产品合同，唯一排除项为 Chromium 专属 CDP heap 压力采样；WebKit 尚未验证 |
