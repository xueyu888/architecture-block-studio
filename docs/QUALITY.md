...
| 证据 | 当前结果 |
| --- | --- |
| TypeScript + Vite production build | 通过；先验证源码架构投影，再完成 1917 modules transformed |
| Vitest 快速单元测试 | 218 / 218 通过，32 个 test files；覆盖桌面文件服务、场景路由、viewport auto-pan、片段、组选区、位置化模块创建、最深 Level drop-target、结构共享几何编辑 / 布局投影、canonical 历史快照、route / line-jump 帧增量、Worker 有界认证帧缓存、latest-only 请求队列、完整候选路线证书，以及真实源码架构门禁：生成文件必须是合法 `BlockDesignDocument`、恰有 5 层、12 个唯一源码模块覆盖 84 个文件、27 条唯一依赖全部带 import 证据 |
...
| Chromium Playwright | Playwright 1.62.1 / Chromium 151.0.7922.34，89 / 89 通过；覆盖完整 renderer 产品合同与 200 / 400 性能旅程 |
| Firefox 产品 Playwright | Playwright 1.62.1 / Firefox build 1538，88 / 88 产品合同通过；执行除 Chromium 专属性能采样外的全部场景。Vite 开发态 inline Worker 在 Firefox 只对 localhost 根入口产生一个固定浏览器 warning，收集器以完整消息和 URL 形状精确识别；production preview 实测为 0 warning，其他 warning / error / pageerror 继续使测试失败 |
...
| 路由五层压力 | 双展开真实场景逐条审计 54 legs、枚举 1431 个无序线对；偏斜 fixture 让一个 5000 × 3500 Hub 拥有 100 个端口和 100 条线、100 个卫星各只有 1 条线，逐条审计 100 routes、枚举 4950 pairs，并在 Optimize Routing 后重跑；纯路由场景覆盖 120 legs / 7140 pairs；真实 UI 五层展开覆盖 20 routes / 190 pairs，并在第 4→5 层键盘重连及 Undo 后重新全量审计；源码架构五层图另覆盖 27 条真实 import 路线与 351 个无序线对，并在选择、resize / Undo、单线交叉定位和保存前后复验；另以 12 个 commodity 穿过五层 Gate 链形成 72 legs / 2556 pairs，检查每条线的 domain 与全部线对；密集共享端口必须诚实 `Unresolved`。逐线检查有限 / 非零 / 正交 / 微短段 / 反向 / 自交，逐对检查共线、8px separation、共享 stub 豁免、每个 strict crossing 的线桥与孤立桥 |
...
| 源码架构截图 | `source-architecture-overview.png` 复核五层边界、12 个真实模块和 27 条 import 依赖的完整总览；`source-architecture-review.png` 复核 Studio 一跳邻域的 9 modules / 8 interfaces 与多选 Inspector；`source-architecture-interface-review.png` 复核单条 `Studio Orchestrator → Model Contract · 2 import declarations` 的完整路径、唯一 target 箭头、端口和接口证据。三图均为 1680 × 1050 真实浏览器流程，0 线中标签、0 穿卡、0 非正交、0 重复重叠、0 无桥交叉或孤立桥 |
...
| 浏览器范围 | Chromium 89 / 89 完整自动化；Firefox 88 / 88 产品合同，唯一排除项为 Chromium 专属性能采样；WebKit 尚未验证，但 Windows 桌面产品不以 WebKit 或移动端为交付目标 |
