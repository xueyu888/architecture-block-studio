...
对齐辅助线沿用同一几何链，但不拥有设计事实。Canvas 在 move / resize 开始时只收集同一父级、当前视口附近的模块矩形，并把 6 CSS px 容差换算为设计坐标；多选移动先由 `alignmentRectBounds` 从全部成员的 absolute rect 冻结一个组包围框，pointer 位移只平移这个 subject，`snapMovingRect` 返回的同一 correction 再等量作用到每个成员。因此吸附结果不依赖用户抓住大卡片还是小卡片，组内相对位置也不会漂移。跨父级混选没有共同候选坐标空间时明确不吸附，不用当前抓取项猜一个目标。`layout/alignmentGuides` 纯函数从 subject 与候选派生边缘、中心及同宽 / 同高结果，`AlignmentGuideLayer` 只渲染当前 gesture 的临时线和尺寸括号。松手后仍只提交既有 `node/move`、`nodes/move`、`fragment/insert` 或 `node/resize`；先以 pointerdown 建立直接 move / resize，再按住 Alt 时，当前 gesture 原样使用用户预览且不显示 guide。Alt 在 pointerdown 前成立则属于选择协议并强制起框。候选不存在、吸附超出尺寸上下限或 Editor 拒绝提交时，不建立补偿状态，Canvas 回到文档投影。
