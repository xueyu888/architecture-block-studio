...
- 使用 ELK 生成分层布局；自动连线始终是源端口到目标端口的一条直接线段。交叉不会触发隐藏的避障或车道算法，选中线路即可高亮并手动改成可保存的正交折线。
...
- 移动或调整模块时默认落在不可见的 16px 设计网格；拖动模块或选择组接近同级邻居的等距位置时，画布优先显示两段无文字间距括号，其次才使用边缘 / 中心对齐，未命中才落网格。先从模块或尺寸抓手开始直接操作，再按住 **Alt** 可同时关闭等距、对齐与网格，进行不受隐藏栅格限制的精确摆放。Alt 若在 pointerdown 前已经按住，则明确表示强制框选，两种能力不会抢占。
...
- 选中任意接口后使用 **Ctrl/⌘ Shift H** 或 **View → Fit Selection**，可把完整线路和两端模块一起聚焦到可读范围，便于逐线人工审查。
...
- 在空白画布右键选择 **Add Module Here**，或把工具栏的 **Add Module** 拖到画布，即可在指定位置创建模块。拖过原位展开的层级时，画布从当前指针下所有可见 Level 表面中只选择最深一层，用蓝色轮廓和最终卡片尺寸预览落位；空子 Level 也有足以容纳一张默认卡片的完整投放表面。提示不叠加文字胶囊，不遮挡模块、端口或线路。对话框只收集名称与 Owner；提交时才吸附 32px 网格并寻找最近无碰撞位置，最终只产生一次 `node/add` 和一次 Undo。若结果已完整可见，视口不会突然跳动；若被边缘、MiniMap 或控件遮住，画布才会留出安全间距将其带回视野。
...
![Windows 桌面直接拖动端口并让线路实时跟随](docs/screenshots/windows-port-direct-placement.png)

![低缩放下仍可直接命中的端口文字](docs/screenshots/windows-port-label-drag.png)

![自动直线与清晰的端口方向](docs/screenshots/direct-line-routing.png)
...
指定位置新建与 Paste Here 共用同一个纯几何放置 Owner，但模块创建不伪装成片段粘贴。Layout 为根 Level 和每个已展开子 Level 发布可见表面与设计原点，Canvas 只负责选择指针下最深的真实表面；Studio 用同一个放置函数生成临时预览，并在对话框确认时针对最新文档重新求解，Editor 才把唯一最终位置与模块事实一起原子写入文档。预览、目标轮廓、指针和层级命中都是可丢弃工作区状态；取消拖放、关闭对话框、非法坐标或未应用的 Inspector 草稿都不会留下半成品。

![空白画布或工具栏拖放指定位置新建模块](docs/screenshots/add-module-here.png)

![五层展开中命中最深子 Level 并预览最终卡片](docs/screenshots/add-module-inline-drop-target.png)

![空子 Level 的完整投放表面与无遮挡预览](docs/screenshots/add-module-empty-level-drop-target.png)
...
选中自动直线后，画布在中点显示一个克制的菱形拖动入口；第一次调整会创建正交手动路线，Inspector 同步显示状态和恢复自动直线的入口。

![手动正交布线与连接 Inspector](docs/screenshots/manual-route-editing.png)

鼠标创建和改接接口使用同一套候选资格。拖动时只有合法端口获得明确提示，预览始终是直接线段且不提前画语义箭头；成功后才由正式连接的 target 表达方向。取消、非法目标和原端点落点都会恢复原设计，并给出不遮挡图形的短反馈。

![鼠标端点改接的候选端口与直接预览](docs/screenshots/pointer-connection-feedback.png)

连接预览与自动线路共用同一个最小合同：两个有限端点、一条直接线段。端口吸附后，正式连接会重新读取规范锚点；JSON 仍只保存模块、端口、接口和用户确认的手动折点，不保存临时指针轨迹。

![实时直接连接预览](docs/screenshots/scene-connection-preview.png)
...
在既有复杂设计中继续新增模块和接口时，线路始终连接真实端口；自动直线可直接选中，再用鼠标或方向键调整为手动折线，最终随同一份 JSON 保存。

![既有复杂设计中的增量布线](docs/screenshots/incremental-routing.png)

连线质量使用五层合同持续检查：浏览器逐条读取全部可见线路，验证有限坐标、非零长度、自动两点和手动正交；五层真实场景检查 20 条线，偏斜压力场景检查单模块连接的全部 100 条线，不抽样。交叉是明确接受的结果，测试不再用旧避障目标反向规定产品行为。

大图操作也不需要等待后台路由求解。移动、缩放和端口拖动只重新投影受当前 React 几何影响的端点；文档、保存和 Undo / Redo 仍只记录用户确认的模块、端口与 waypoint 事实。
...
4. 之后可从 **File → Recent Designs** 重开最近文件；使用 **Save** 原位保存，使用 **Save As** 保留新副本。磁盘写入成功前不会清除未保存状态。
5. 在标题栏切换五种界面语言；左右栏可从 Header 隐藏，并点击窄边栏恢复。
6. 首次安装后可直接从标题栏检查新版本、下载并查看进度；下载完成后点击“重启并安装”。存在未保存设计或未应用 Inspector 草稿时，客户端会拒绝重启，先保护内容再更新。

![五种界面语言、可折叠侧栏与最近设计菜单](docs/screenshots/windows-multilingual-recent-designs.png)
...
- **最近设计**：成功打开或保存后会进入本机最近列表；条目只引用原文件，不缓存第二份设计内容。文件被移动或删除时，重开会明确失败并清理条目。
...
这份示例不是手绘宣传图。生成器会遍历 `src` 中的 TypeScript、TSX 与 CSS 文件，要求每个文件恰好属于一个责任模块，并把所有可解析的跨模块相对 import（包括 Vite 资源查询）投影为接口；当前示例覆盖 70 个源码文件、12 个责任模块、30 条跨模块依赖和 5 层上下文。缺失归属、虚构依赖、无法解析的引用或模块环都会使验证失败。
...
面对五层源码图时，不必把所有父级容器都挤在一个缩放视图里。连续进入模块后，第五层 12 个真实源码模块和 30 条依赖可以独占画布；顶部 breadcrumb 始终保留完整来路，退出时还会重新选中拥有该子设计的父模块。
...
- [连线策略](docs/ROUTING.md)：自动直线、手工 waypoint、端点方向、交互编辑与逐线验证合同。
